...
| **US1**        |  **FR1, FR2**      | Filter by hygiene rating 5.| **Pass**|
 ...
 |  **UC1**        |  **FR2**           | Display business details on marker click.| **Fail** |
  |  **UC2**        |  **FR4**           | Submit issue form .| **Pass** |
...
|   **UC1**        |  **NFR1, NFR2**    | Mobile responsiveness for dropdown.| **Pass** |
  | **UC3**        |  **FR6, NFR1, NFR4** | API fallback activates if API fails. | **Fail** |
  | **UC3**    |  **FR6, NFR1**       | System switches to GeoJSON backup data. |  **Pass**      |
| **UC1** | **FR3, FR4** | Toggle between Map View and List View retains search parameters.| **Fail** |
...
| **UC1** | **FR8** | List View updates results instantly without page refresh. | **Fail** | 
